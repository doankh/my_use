...
// $Id: NewObjectDiagram.java 6300 2018-01-14 12:22:29Z andreask $
...
public class NewObjectDiagram extends DiagramViewWithObjectNode implements HighlightChangeListener, SortChangeListener {
	...
	public void hideAllLinks() {
		/*
		 * this.hiddenData.fBinaryLinkToEdgeMap.putAll(this.visibleData.
		 * fBinaryLinkToEdgeMap);
		 * this.hiddenData.fHalfLinkToEdgeMap.putAll(this.visibleData.
		 * fHalfLinkToEdgeMap);
		 * this.hiddenData.fLinkObjectToNodeEdge.putAll(this.visibleData.
		 * fLinkObjectToNodeEdge);
		 * this.hiddenData.fNaryLinkToDiamondNodeMap.putAll(this.visibleData.
		 * fNaryLinkToDiamondNodeMap);
		 * 
		 * this.visibleData.fBinaryLinkToEdgeMap.clear();
		 * this.visibleData.fHalfLinkToEdgeMap.clear();
		 * this.visibleData.fLinkObjectToNodeEdge.clear();
		 * this.visibleData.fNaryLinkToDiamondNodeMap.clear();
		 */

		// ConcurrentHashMap <MLink, BinaryAssociationOrLinkEdge> tmp = new
		// ConcurrentHashMap<>();

		// Hide BinaryLinks
		// create temporary Map for looping and removing (Concurreny)
		/*
		 * Map<MLink, BinaryAssociationOrLinkEdge> binaryLooping = new
		 * HashMap<MLink, BinaryAssociationOrLinkEdge>(); binaryLooping
		 * .putAll(this.visibleData.fBinaryLinkToEdgeMap);
		 * for(Map.Entry<MLink,BinaryAssociationOrLinkEdge> e :
		 * binaryLooping.entrySet()){ hideBinaryLink(e.getKey()); }
		 * binaryLooping.clear();
		 */
		// Hide HalfLinks (Was ist das?)
		// used by TernaryAssoc
		/*
		 * Map<MLink, List<EdgeBase>> halfLinkLooping = new HashMap<MLink,
		 * List<EdgeBase>>(); halfLinkLooping
		 * .putAll(this.visibleData.fHalfLinkToEdgeMap);
		 * for(Map.Entry<MLink,List<EdgeBase>> e : halfLinkLooping.entrySet()){
		 * System.err.println("Hide Halflink");
		 * System.err.println("Class: "+e.getClass());
		 * System.err.println("Key: "+e.getKey());
		 * System.err.println("Value: "+e.getValue()); hideLink(e.getKey()); }
		 * halfLinkLooping.clear();
		 * 
		 * //Hide LinkObject (Was ist das?) //used by Association class
		 * Map<MLink, EdgeBase> linkObjectToNodeLooping = new HashMap<MLink,
		 * EdgeBase>(); linkObjectToNodeLooping
		 * .putAll(this.visibleData.fLinkObjectToNodeEdge); for(Entry<MLink,
		 * EdgeBase> e : linkObjectToNodeLooping.entrySet()){
		 * System.err.println("Hide LinkObject");
		 * System.err.println("Class: "+e.getClass());
		 * System.err.println("Key: "+e.getKey());
		 * System.err.println("Value: "+e.getValue()); hideLink(e.getKey()); }
		 * linkObjectToNodeLooping.clear();
		 * 
		 * //Hide NaryLinks //create temporary Map for looping and removing
		 * (Concurreny) Map<MLink, DiamondNode> naryLooping = new
		 * HashMap<MLink,DiamondNode>(); naryLooping
		 * .putAll(this.visibleData.fNaryLinkToDiamondNodeMap); for(Entry<MLink,
		 * DiamondNode> e : naryLooping.entrySet()){ hideNAryLink(e.getKey()); }
		 * naryLooping.clear();
		 */
		...
		// Hide LinkObject (Was ist das?)
		// used by Association class
		...
		/*
		 * Set<EdgeBase> edges = this.visibleData.getEdges(); for(EdgeBase edge
		 * : edges){ System.err.println(edge.getId());
		 * System.err.println(edge.getName());
		 * System.err.println(edge.getClass());
		 * 
		 * edge.setHidden(true); }
		 */
	...
	/*
	 * public void hideLink2(EdgeBase e) { // e.setHidden(true);
	 * 
	 * System.err.println(e.getId()); String aa = (e.getId().split("::"))[0];
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.keySet().
	 * contains(aa));
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.keySet().
	 * toString().contains(aa)); System.err.println(aa);
	 * 
	 * System.err.println(e.getClass()); System.err.println(e.getName());
	 * System.err.println(e.getOffset());
	 * System.err.println(e.getSourceHeightHint());
	 * System.err.println(e.getTargetHeightHint());
	 * System.err.println(e.getProperties());
	 * System.err.println(e.getWayPoints().toString());
	 * 
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.toString());
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.keySet());
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.keySet().
	 * toString());
	 * System.err.println(this.visibleData.fHalfLinkToEdgeMap.values());
	 * System.err.println("aaaaaaaaaaaaaaa");
	 * System.err.println(this.visibleData.fLinkObjectToNodeEdge.keySet());
	 * 
	 * 
	 * if (e instanceof LinkEdge) { LinkEdge aEdge = (LinkEdge) e;
	 * System.err.println("E-Class: " + e.getClass());
	 * 
	 * MLink link = aEdge.getLink(); System.err.println("LINK: " +
	 * link.toString()); System.err.println("CHECK: " +
	 * this.visibleData.fHalfLinkToEdgeMap.keySet().contains(link));
	 * System.err.println("CLASS: " + link.getClass());
	 * System.err.println("QUALIFIER: " + link.getQualifier().toString());
	 * System.err.println("ASSOC: " + link.association());
	 * System.err.println("ASSOC_TO_STRING: " + link.association().toString());
	 * System.err.println("ASSOC_TO_Model: " + link.association().model());
	 * 
	 * hideLink(link); }
	 * 
	 * 
	 * Set<MLink> links = new HashSet<>(); if
	 * (this.visibleData.fBinaryLinkToEdgeMap.containsValue(e)){
	 * 
	 * links.addAll(this.visibleData.fBinaryLinkToEdgeMap.keySet()); for(MLink
	 * link : links){ if
	 * (this.visibleData.fBinaryLinkToEdgeMap.get(link).equals(e)){
	 * hideBinaryLink(link); } } links.clear(); }
	 * 
	 * 
	 * if (this.visibleData.fHalfLinkToEdgeMap.containsValue(e) ||
	 * this.visibleData.fLinkObjectToNodeEdge.containsValue(e)){
	 * 
	 * links.addAll(this.visibleData.fHalfLinkToEdgeMap.keySet());
	 * links.addAll(this.visibleData.fLinkObjectToNodeEdge.keySet());
	 * 
	 * for(MLink link : links){ if
	 * (this.visibleData.fHalfLinkToEdgeMap.get(link).equals(e) ||
	 * this.visibleData.fLinkObjectToNodeEdge.get(link).equals(e)){
	 * hideLink(link); } } links.clear(); }
	 * 
	 * if (this.visibleData.fNaryLinkToDiamondNodeMap.containsValue(e)){
	 * 
	 * links.addAll(this.visibleData.fNaryLinkToDiamondNodeMap.keySet());
	 * for(MLink link : links){ if
	 * (this.visibleData.fNaryLinkToDiamondNodeMap.get(link).equals(e)){
	 * hideNAryLink(link); } } links.clear(); }
	 * 
	 * 
	 * 
	 * // this.visibleData.containsLink(e.getId()); }
	 */
	...
	public void showAllLinks() {
		 ...
		 * Set<MLink> links = new HashSet<>();
		 * 
		 * links.addAll(this.hiddenData.fBinaryLinkToEdgeMap.keySet());
		 * 
		 * for (MLink e : links) { showBinaryLink(e); }
		 * 
		 * links.clear();
		 * 
		 * // Hide HalfLinks (Was ist das?) // used by TernaryAssoc
		 * links.addAll(this.hiddenData.fHalfLinkToEdgeMap.keySet());
		 * 
		 * // Hide LinkObject (Was ist das?) // used by Association class
		 * links.addAll(this.hiddenData.fLinkObjectToNodeEdge.keySet()); for
		 * (MLink e : links) { showObjects(new HashSet<>(e.linkedObjects()));
		 * showLink(e); }
		 * 
		 * links.clear();
		 * 
		 * // Hide NaryLinks
		 * links.addAll(this.hiddenData.fNaryLinkToDiamondNodeMap.keySet()); for
		 * (MLink e : links) { showNAryLink(e); }
		 * 
		 * links.clear();
		 */ }
	...
	public void hideLink(MLink link) {
		...
		if (link.linkEnds().size() == 2) {
			...
			/*System.err.println("Check:"+ (link instanceof MLinkObject));
			System.err.println("Check-fLinkObjectToNodeEdge:"+visibleData.fLinkObjectToNodeEdge.get(link));
			EdgeBase e = visibleData.fLinkObjectToNodeEdge.get(link);
			
			BinaryAssociationClassOrObject edge = (BinaryAssociationClassOrObject) visibleData.fLinkObjectToNodeEdge.get(link);
		
			//EdgeBase simpleEdge = (EdgeBase) edge.getAssociationClassEdge();
			 EdgeBase simpleEdge = edge.getAssociationClassEdge();
			System.err.println("SimpleEdge: "+simpleEdge.toString());
//			System.err.println("Try remove edge: "+fGraph.removeEdge(simpleEdge));
			edge.removeWayPoint(simpleEdge.getWayPoints().iterator().next());
			*/
			hideBinaryLink(link);
		} else {
			hideNAryLink(link);
	...
	protected PopupMenuInfo unionOfPopUpMenu() {
		...
		// Ab hier neues selected-Handling
		if (!selectedLinks.isEmpty() || !selectedObjects.isEmpty() || !selectedObjectsOfAssociation.isEmpty()) {
			...
			if (size > 1) {
				...
				labelHide = "Hide " + selectedLinks.iterator().next().association().name();
				labelCrop = "Crop " + selectedLinks.iterator().next().association().name();
			} else if (selectedObjects.size() == 1) {
				labelHide = "Hide " + selectedObjects.listIterator().next().name();
				labelCrop = "Crop " + selectedObjects.listIterator().next().name();
			}
			/*
			 * else if (selectedObjectsOfAssociation.size() == 1){ labelHide =
			 * "Hide " + selectedObjectsOfAssociation.iterator().next().name();
			 * labelCrop = "Crop " +
			 * selectedObjectsOfAssociation.iterator().next().name(); }
			 */
			/*
			 * if (!selectedObjects.isEmpty()){ if (selectedObjects.size() ==
			 * 1){ labelGreyOut = (selectedObjects.iterator().next().isGreyOut()
			 * ? "Grey in" : "Grey out") +
			 * selectedObjects.iterator().next().name(); }else {
			 * 
			 * } }
			 */
			// Erstelle Action fuer Hide
			popupMenu.insert(new AbstractAction(labelHide) {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					/*
					 * if(selectedObjectsOfAssociation.size() > 0){
					 * selectedObjectsOfAssociation.forEach(obj ->
					 * hideObject(obj)); }
					 */
					repaint();
				}
			...
			// erstelle Action fuer crop
			Set<MObject> objectsToHide = new HashSet<>();
			objectsToHide.addAll(selectedObjects);
			// objectsToHide.addAll(selectedObjectsOfAssociation);
		...
		// Ende: neues selected handling

		/*
		 * if (selectedObjects.isEmpty() && selectedObjectsOfAssociation.size()
		 * > 0) { String info;
		 * 
		 * if (selectedLinks.size() == 1) { info =
		 * selectedLinks.iterator().next().association().name(); } else { info =
		 * "" + selectedLinks.size() + " links"; }
		 * 
		 * popupMenu.insert(new AbstractAction("Hide " + info) {
		 * 
		 * @Override public void actionPerformed(ActionEvent e) { for (MLink
		 * linkToHide : selectedLinks) { hideLink(linkToHide); repaint(); } } },
		 * pos++);
		 * 
		 * popupMenu.insert(new AbstractAction("Crop " + info) {
		 * 
		 * @Override public void actionPerformed(ActionEvent e) {
		 * hideAllLinks(); hideAll();
		 * 
		 * // %TODO: Liste als Set for (MLink link : selectedLinks) {
		 * showLink(link); }
		 * 
		 * repaint(); } }, pos++);
		 * 
		...
		/*
		 * if (!selectedObjects.isEmpty()) { popupMenu.insert(getAction("Hide "
		 * + selectedObjectsText, selectedObjectsSet), pos++);
		 * popupMenu.insert(getAction("Crop " + selectedObjectsText,
		 * getNoneSelectedNodes(selectedObjectsSet)), pos++);
		 * 
		 * popupMenu.insert(new JSeparator(), pos++); }
		 */
		...
		if (!visibleData.getEdges().isEmpty() || !hiddenData.getEdges().isEmpty()){
			popupMenu.insert(fSelection.getSubMenuAssociation(), pos++);
	...
	private Set<MObject> getNoneSelectedNodes(Set<MObject> selectedNodes) {
		...
		selectedNodes.forEach(elem -> {
			if (elem instanceof MLinkObjectImpl) {
				// Set<MObject> clss = ((MLinkObjectImpl)
				// elem).association().associatedClasses();
	...
	public void hideLink(String associationName, List<MLink> links) {
		for ( MLink link : links) {
			if (associationName.equals(link.association().toString())) {
				hideLink(link);
			}
		}
	...
	public void showLink(String associationName, List<MLink> links) {
		for (MLink link : links) {
			if (associationName.equals(link.association().toString())) {
				showLink(link);
			}
	...
	public TreeMap<String, List<MLink>> getLinksOfAssocs() {
		...
		final String aggreation = "Aggreation";
		final String compositon = "Compositon";
		...
		for (MAssociation assoc : fParent.system().model().associations()) {
			...
			case 0:
				...
				for (MLink link : links) {
					if (MAssociationClassImpl.class.isInstance(link.association())) {
					...
					} else if (MAssociation.class.isInstance(link.association()) && (new HashSet<MObject>(link.linkedObjects()).size() == 1) ) {
			...
			case 1:
				if (!fParent.system().state().linksOfAssociation(assoc).links().iterator().hasNext())
					break;
				MLink agg = fParent.system().state().linksOfAssociation(assoc).links().iterator().next();

				if (!assocs.containsKey(aggreation)) {
					assocs.put(aggreation, new ArrayList<MLink>() {
						{
							add(agg);
						}
					});
				} else {
					assocs.get(aggreation).add(agg);
				}
				break;
			...
			case 2:
				if (!fParent.system().state().linksOfAssociation(assoc).links().iterator().hasNext())
					break;
				MLink comp = fParent.system().state().linksOfAssociation(assoc).links().iterator().next();
	...
	public void showLink(List<MLink> links) {
		...
		for (MLink link : links) {
			// showObjects(new HashSet<>(link.linkedObjects()));
			showLink(link);
		}
	}
